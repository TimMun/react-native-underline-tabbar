...
import { Text, TouchableOpacity, View, Animated, ScrollView, StyleSheet } from 'react-native';
...
class TabBar extends Component<Props, State> {
  ...
  renderTab = (
    ...
    // If previous tab has a badge, subtract the width of the badge
    const badgeMarginLeft = StyleSheet.flatten([styles.badgeBubble, this.props.tabStyles.badgeBubble]).marginLeft;
    const badgeWidth = StyleSheet.flatten([styles.badgeBubble, this.props.tabStyles.badgeBubble]).width;
    const badgeMarginAdjustment = prevTab && prevTab.badge && parseInt(prevTab.badge) > 0 ? badgeWidth + badgeMarginLeft : 0;

    return (
      <TouchableOpacity
        style={[
          styles.tab,
          this.props.tabMargin && { marginLeft: this.props.tabMargin - badgeMarginAdjustment },
      ...
      >
        ...
        >
          {tabs.map((tab: TabType, page: number) => {
            const prevTab = page > 0 ? tabs[page - 1] : null;
            ...
            return renderTab(prevTab, tab, page, isTabActive, onPressHandler, onTabLayout);
          })}
